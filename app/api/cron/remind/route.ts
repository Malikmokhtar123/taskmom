...
export async function GET() {
  ...
  if (apiKey) {
    ...
    const supplyList = lowSupplies
      .map(s => `${s.child_name}: ${s.type.replace(/_/g, ' ')} — ${s.days_remaining} day${s.days_remaining === 1 ? '' : 's'} left`)
      .join('\n');
    const res = await client.chat.completions.create({
      ...
      messages: [{
        role: 'user',
        content: `Write a warm, brief SMS reminder for a mom managing T1D supplies for her 5 kids. These need reordering:\n${supplyList}\n\nMax 160 characters. Caring and direct. Start with "TaskMom:".`,
      }],
    });
    message = res.choices[0].message.content ?? '';
  } else {
    const lines = lowSupplies.map(s => `• ${s.child_name}: ${s.days_remaining}d left`);
    message = `TaskMom Daily Check\n\n${lines.join('\n')}\n\nReorder now.`;
  }
